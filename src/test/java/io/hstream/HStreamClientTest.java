...
public class HStreamClientTest {
    ...
    public void testStreamQuery() {
        ...
        Producer producer = client.newProducer().stream(TEST_STREAM).build();
        HRecord hRecord1 = HRecord.newBuilder()
                .put("temperature", 29)
                .put("humidity", 20)
                .build();
        HRecord hRecord2 = HRecord.newBuilder()
                .put("temperature", 34)
                .put("humidity", 21)
                .build();
        HRecord hRecord3 = HRecord.newBuilder()
                .put("temperature", 35)
                .put("humidity", 22)
                .build();
    ...
    public void testWriteBatchRawRecord() {
        ...
        Producer producer = client.newProducer().stream(TEST_STREAM).enableBatch().recordCountLimit(10).build();
        Random random = new Random();
        final int count = 100;
    ...
    public void testFlush() {
        ...
        Producer producer = client.newProducer().stream(TEST_STREAM).enableBatch().recordCountLimit(100).build();
        Random random = new Random();
        final int count = 10;
        CompletableFuture<RecordId>[] recordIdFutures = new CompletableFuture[count];
        for(int i = 0; i < count; ++i) {
