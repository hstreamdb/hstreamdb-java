...
public class RecordUtils {
    ...
    public static HStreamRecord buildHStreamRecordFromRawRecord(byte[] rawRecord) {
        ...
        return HStreamRecord.newBuilder().setHeader(header).setPayload(ByteString.copyFrom(rawRecord)).build();
    }

    public static HStreamRecord buildHStreamRecordFromHRecord(HRecord hRecord) {
        try{
            HStreamRecordHeader header = HStreamRecordHeader.newBuilder().setFlag(JSON_RECORD_FLAG).build();
            String json = JsonFormat.printer().print(hRecord.getDelegate());
            logger.debug("hrecord to json: {}", json);
            return HStreamRecord.newBuilder().setHeader(header).setPayload(ByteString.copyFrom(json, StandardCharsets.UTF_8)).build();
